"""
GPU (Intel XPU) Inference 벤치마크
===================================

YOLO11n-OBB 모델을 Intel Arc A770 (XPU)에서 추론하는 속도를 측정합니다.
torch.xpu.synchronize()를 사용하여 비동기 연산 완료를 보장합니다.
...
from src.detection.constants import DEVICE  # XPU 패치 + 디바이스 감지
...
def run_benchmark(runs=50, warmup=10, img_path=None):
    """GPU (Intel XPU) 벤치마크를 실행합니다."""
    import cv2

    # XPU 확인
    if not (hasattr(torch, 'xpu') and torch.xpu.is_available()):
        print("Intel XPU를 사용할 수 없습니다.")
        print("   torch.xpu.is_available() = False")
        sys.exit(1)

    gpu_name = torch.xpu.get_device_name(0)
    vram_bytes = torch.xpu.get_device_properties(0).total_memory
    ...
    print(f"디바이스: XPU ({gpu_name}, {vram_mb:.0f}MB VRAM)")
    model = YOLO(model_path)

    # Warm-up (XPU 커널 컴파일 + VRAM 할당 안정화)
    print(f"\nWarm-up ({warmup}회)...")
    for i in range(warmup):
        model(image, device=DEVICE, verbose=False)
        torch.xpu.synchronize()
    print("   Warm-up 완료")

    # 본 측정 (XPU sync로 정밀 측정)
    print(f"\n벤치마크 실행 ({runs}회)...")
    times = []
    for i in range(runs):
        torch.xpu.synchronize()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GPU (Intel XPU) Inference Benchmark")
